  ...
  useEffect(() => {
    const startChat = async () => {
      ...
      try {
        ...
        setMessages([{ role: 'ai', content: "Error starting conversation. Please try again." }]);
      } finally {
        setIsLoading(false);
      }
    };
    startChat();
  }, [topic, level, sessionId]);
  ...
  if (showReport) {
    return (
      <div className="max-w-2xl mx-auto p-4">
        <button onClick={() => setShowReport(false)} className="mb-4 text-blue-600 hover:underline">
          ← Back to Chat
        </button>
